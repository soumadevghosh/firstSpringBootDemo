...
public class LoggingAspect //crosscutting concerns
{
	...
	{
		LOGGER.info("before getAliens is called");			// write in the log
	}
	
	
//	@AfterReturning 	// executed after successful execution only
//	@AfterThrowing		// only when we get an exception
	@After("execution(public * com.SpringMVCBoot.controller.AlienController.getAliens())")   // executed after Finally no matter what error is there 
	public void logAfter() 
	{
		LOGGER.info("after getAliens is called");			// write in the log
	}
}
//have to add a dependency if it's not a spring boot app
//6.1.1. AOP concepts
/*
 * Let us begin by defining some central AOP concepts and terminology. These
 * terms are not Spring-specific... unfortunately, AOP terminology is not
 * particularly intuitive; however, it would be even more confusing if Spring
 ...
 * as the advised object. Since Spring AOP is implemented using runtime proxies,
